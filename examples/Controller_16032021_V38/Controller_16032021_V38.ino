...
/*
// This section is only valid for Arduino Uno due to its digital pin 4 is not working
// due to the sliding switch was in contact with the voltage step down XL4016 yesterday (23.04.2021)
Joystick joystick1(1,0);
Switch switchcontroll er(2);
Switch switchcutter(3);
Switch switchautoforward(5);  // Digital pin 4 is not responsive
Potentiometer potentiocutter(4);// Analog pin
LED switchcontrollerLED(6);
LED switchcutterLED(7);
LED switchautoforwardLED(8);
//LED testport4LED(4);  // To test if the digital port 4 still working
*/

// This section is only valid for Elegoo Uno because its digital pin 4 has no problem
Joystick joystick1(1,0);
...
switchcontroller.setSwitchStatus(); // Sliding switch's position will be read
if(switchcontroller.getSwitchStatus() == MOWER::ON) // The controller will only function if it's turned on
  {
    ...
    /***** To test if the digital port 4 still working *****/
    /*testport4LED.setLEDStatus(static_cast<MOWER::ONOFF_Status>(autoforwardswitchStatus));
    Serial.print("testport4LED: ");
    Serial.println(testport4LED.getLEDStatus());*/
    // Result -> Port 4 is not working anymore
    ...
    */
    ...
    tx_buf[4] = cutterSpeed;  // Should be a random value because no value has been assigned to the cutterSpeed.
...
else  // All these switches need to be forcefully to turn off, so that its LED will also be turned off
  {
    switchcutter.setSwitchForceShutDown();
    switchautoforward.setSwitchForceShutDown();   
  } 
