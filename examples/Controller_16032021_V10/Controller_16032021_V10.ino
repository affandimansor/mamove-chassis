...
#include <LED.h>
#include <Mower.h>
#include "NRF24L01_node.h"  // 'NRF24L01_controller.h' needs to be placed inside an individual folder, otherwise an error always pops out here when we compile this sketch
...
//tx_buf[0] = joystick1.getjoystickPosX(); //<-- To be transmitted over radio communication to chassis!!!
//tx_buf[1] = joystick1.getjoystickPosY(); //<-- To be transmitted over radio communication to chassis!!!
...
JoystickPosX = map(analogRead(joystick1.getjoystickPosX()),0,1023,0,255);
JoystickPosY = map(analogRead(joystick1.getjoystickPosY()),0,1023,0,255);
...
cutterswitchStatus = 2;//switchcutter.getSwitchStatus();  //<-- To be transmitted over radio communication to chassis!!!
